...
var FONT_STACK = "'Plus Jakarta Sans', 'Hanken Grotesk', -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif";
var GOLD = "#FEC12C";
var DARK = "#1a1c1c";
var BODY_BG = "#f3f3f1";
var CARD_BG = "#ffffff";
var TEXT_PRIMARY = "#1a1c1c";
var TEXT_SECONDARY = "#5a5d5e";
var TEXT_MUTED = "#8a8d8e";
var BORDER = "#e8e8e5";
var LOGO_URL = "https://styris.sk/logo.png";
var SITE_URL = "https://styris.sk";
...
function buildUserEmail(name, projectType) {
	return `<!DOCTYPE html>
...
Ďakujeme, ${escapeHtml(name.split(/\s+/)[0] || name)}!
</td>
</tr>
...
Vašu správu ohľadom <strong style="color:${TEXT_PRIMARY};">${escapeHtml(projectTypeLabel(projectType))}</strong> sme úspešne prijali. Náš tím ju práve posudzuje a&nbsp;ozveme sa vám <strong style="color:${TEXT_PRIMARY};">do 24 hodín</strong>.
</td>
</tr>
...
function buildAdminEmail(name, email, projectType, message, phone) {
	const safeName = escapeHtml(name);
	const safeEmail = escapeHtml(email);
	const safeType = escapeHtml(projectTypeLabel(projectType));
	const safeMessage = escapeHtml(message).replace(/\n/g, "<br>");
	const safePhone = phone ? escapeHtml(phone) : "—";
	const mailtoLink = `mailto:${email}?subject=${encodeURIComponent(`Re: Dopyt — ${projectType || "Nový projekt"}`)}&body=${encodeURIComponent(`Dobrý deň ${name},
...
${(/* @__PURE__ */ new Date()).toLocaleDateString("sk-SK", {
		day: "numeric",
		month: "long",
		year: "numeric",
		hour: "2-digit",
...
var POST = async ({ request }) => {
	try {
		const { name, email, type, message, phone } = await request.json();
		if (!name?.trim() || !email?.trim() || !type?.trim() || !message?.trim()) return new Response(JSON.stringify({ error: "Vyplňte všetky povinné polia." }), {
			status: 400,
			headers: { "Content-Type": "application/json" }
		});
		if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email.trim())) return new Response(JSON.stringify({ error: "Zadajte platnú emailovú adresu." }), {
			status: 400,
			headers: { "Content-Type": "application/json" }
		});
		const env = typeof process !== "undefined" && process.env || {};
		const apiKey = env.MAILJET_API_KEY || "80a7e1fa8912a72a3fd66f916098cb6b";
		const apiSecret = env.MAILJET_API_SECRET || "d906923495b29ff7837b0236e84cdad5";
		const fromEmail = env.MAILJET_FROM_EMAIL || "info@styris.sk";
		const adminEmails = (env.ADMIN_EMAILS || "styris.sk@gmail.com").split(",").map((e) => e.trim()).filter(Boolean);
		if (!apiKey || !apiSecret) {
			console.error("[contact] Missing Mailjet API credentials");
			return new Response(JSON.stringify({ error: "Chyba servera. Skúste to prosím neskôr." }), {
				status: 500,
				headers: { "Content-Type": "application/json" }
			});
		}
		...
		const adminHtml = buildAdminEmail(trimmedName, trimmedEmail, trimmedType, trimmedMessage, trimmedPhone);
		const messages = [{
			From: {
				Email: fromEmail,
				Name: "Styris"
...
styris.sk`
			...
			From: {
				Email: fromEmail,
				Name: "Styris — Kontaktný formulár"
			},
			To: [{ Email: adminEmail }],
			ReplyTo: {
				Email: trimmedEmail,
...
Odoslaná cez kontaktný formulár na styris.sk`
		...
		if (!mjRes.ok) {
			...
			return new Response(JSON.stringify({ error: "Nepodarilo sa odoslať správu. Skúste to prosím neskôr." }), {
				status: 502,
				headers: { "Content-Type": "application/json" }
			});
		}
